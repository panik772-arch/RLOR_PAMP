...
class Decoder(nn.Module):
    ...
    def advance(self, cached_embeddings, state):
        ...
        # [batch, 1, context dim]
        context = self.context(node_embeddings, state) #(batch,n_traj, context_dim) e.g. (1024,35,129) or (1024,35,130) when in cvrp_fleet_env
        step_context = self.project_step_context(context)  # (batch, n_traj, embed_dim) e.g. (1024,35,128)
        query = graph_context + step_context  # graph_context (batch, 1, embed) e.g (1024, 1, 128) + step_context -> (batch, n_traj, embed_dim) e.g. (1024,35,128)
        ...
        # Compute the mask
        mask = state.get_mask() #in cvrp_env (1024,50,51) where False is visited and True is to visit)

        # Compute logits (unnormalized log_p)
        logits, glimpse = self.calc_logits(query, glimpse_K, glimpse_V, logit_K, mask, state)

        return logits, glimpse

    def calc_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, state = None):
        ...
        logits = self.pointer(glimpse,  logit_K, mask,  state = state) # in cvrp_env -> with mask in decoder e receive attentScrore of the shape (1025,50,51) as same as mask shape! with values like [-9,89, -inf, -inf...]. So depot is not -inf and all other -inf,,why?
