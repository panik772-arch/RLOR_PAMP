...
class PrevNodeContext(nn.Module):
    ...
    def _prev_node_embedding(self, embeddings, state):
        current_node = state.get_current_node() # (batch, n_traj)
        prev_node_embedding = _gather_by_index(embeddings, current_node) # (batch, n_traj, embed)
    ...
    def forward(self, embeddings, state):
        prev_node_embedding = self._prev_node_embedding(embeddings, state) # in  cvrp conext: (1024,n_traj,128)
        state_embedding = self._state_embedding(embeddings, state) # in cvrp_env (batch,n_traj,1)
        # Embedding of previous node + remaining capacity

        #zeros_padding = torch.zeros(prev_node_embedding.shape[0], 1, prev_node_embedding.shape[2],
        #                            dtype=prev_node_embedding.dtype, device=prev_node_embedding.device)
...
class CVRPFleetContext(PrevNodeContext):
    ...
    def __init__(self, context_dim):
        super(CVRPFleetContext, self).__init__(context_dim)
