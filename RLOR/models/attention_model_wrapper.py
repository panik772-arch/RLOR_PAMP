...
class Critic(nn.Module):
    ...
    def forward(self, x): #glimpse! not logits!
...
class stateWrapper:
    ...
    def __init__(self, states, device, problem="cvrp_fleet", k = 50):
        self.device = device
    ...
    def tw_ratio(self):
        ...
        num_traj = traveled_time.shape[1]
        deadlines = deadline.unsqueeze(1).expand(-1, num_traj, -1) - traveled_time.unsqueeze(-1)
        num_batches = deadlines.shape[0] # add depots
    ...
    def times_to_reach_all_nodes(self):
        ...
        Returns:
            ...
            times_requested - just tw from env (traj, N)
            traveled_time - (B,traj)
            calculate_time_from_this_to_all - time matrix (5,1,11)
        ...
        calculate_time_from_this_to_all = self.calculate_distance_to_all_nodes()/v #(5,1,11)

        updated_ = calculate_time_from_this_to_all + traveled_time.unsqueeze(-1)
        updated_times = torch.where(updated_<1, updated_, 1) #1 == not reachable, if 1, max time. then is not reachable

        tw = self.get_tw()
        num_batches = tw.shape[0]
        zeros = torch.zeros((num_batches, 1), dtype=tw.dtype, device=tw.device)
        times_requested = torch.cat((zeros, tw), dim=1) #(batch, 11) with depot = 0

        # now calculate the difference between required times and . for small values high relevance.
        # for high values small. 0 - not reachable
        difference_factor = times_requested.unsqueeze(1).expand(-1, traveled_time.size(1),-1) - updated_times

        # 5/(5x+1) ratio has better slope then ln function.if smaller zero, than penalize with -10
        ratio = torch.where(difference_factor > 0, self.ATTENTION_SCALING / (self.ATTENTION_SCALING * difference_factor + 1), -10)
        # epsilon = 1e-6
        # transformed_times = -torch.log(ratio + epsilon)

        return ratio,difference_factor,times_requested,traveled_time,calculate_time_from_this_to_all #transformed_times,ratio,times_after_step
    ...
    def return_topK_closest(self):
        ...
        k = self.k
        if k is None:
            k = int(len(self.states["observations"]["loc"][0]))

        dist_matrix = self.calculate_distance_to_all_nodes()
        ...
        return transformed_distances, dist_matrix

    def get_current_node(self):
        return self.states["last_node_idx"]

    def get_tw(self):
        return self.states["tw"]

    def get_traveled_dist(self):
        #(num_traj,)
        return self.states["traveled_dist"]
