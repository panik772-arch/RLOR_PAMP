...
if __name__ == "__main__":
    ...
    os.makedirs("/content/drive/MyDrive/Colab Notebooks/RLOR/" +  f"runs/{run_name}" + "/ckpt",exist_ok=True)
    shutil.copy(__file__, "/content/drive/MyDrive/Colab Notebooks/RLOR/" +  f"runs/{run_name}/main.py")
...
#     test_envs = SyncVectorEnv([make_env(args.env_id, args.seed + args.num_envs + i) for i in range(args.n_test)])
    ...
    for update in range(1, num_updates + 1):
        ...
        if update % 10 == 0 or update == num_updates:
            torch.save(agent.state_dict(), f"/content/drive/MyDrive/Colab Notebooks/RLOR/runs/{run_name}/ckpt/{update}.pt")
